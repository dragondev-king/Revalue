...
1.  Remove statement `import { makeSelectPropertyLocationState } from 'containers/App/selectors'`
2.  Make necessary changes to `history` as follows:

```js
const makeSelectPropertyLocationState = () => {
...
const history = syncHistoryWithStore(browserHistory, store, {
  selectLocationState: makeSelectPropertyLocationState(),
});
```
